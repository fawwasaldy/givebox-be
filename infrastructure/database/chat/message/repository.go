...
func (r repository) Update(ctx context.Context, tx interface{}, messageEntity message.Message) (message.Message, error) {
	validatedTransaction, err := validation.ValidateTransaction(tx)
	if err != nil {
		return message.Message{}, err
	}
	...
	messageSchema := EntityToSchema(messageEntity)
	if err = db.WithContext(ctx).Updates(&messageSchema).Error; err != nil {
		return message.Message{}, err
	}
...
func (r repository) Delete(ctx context.Context, tx interface{}, id string) error {
	...
	if err = db.WithContext(ctx).Where("id = ?", id).Delete(&Message{}).Error; err != nil {
		return err
	}
	return nil
}
